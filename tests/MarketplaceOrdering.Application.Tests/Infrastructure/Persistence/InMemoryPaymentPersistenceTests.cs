...
public sealed class InMemoryPaymentPersistenceTests
{
    ...
    public async Task TransactionClaimAndPaymentSaveAreAtomicAndGlobal()
    {
        ...
        var winner = await repository.SavePaymentAsync(
            first, transactionId, default);
        var loser = await repository.SavePaymentAsync(
            second, transactionId, default);
        ...
        (await repository.LoadAsync(second.Id, default)).Value.Status
    ...
    public async Task VersionConflictDoesNotClaimTransaction()
    {
        ...
        var stale = (await repository.LoadAsync(original.Id, default)).Value;
        var winner = (await repository.LoadAsync(original.Id, default)).Value;
        winner.ChangeItemQuantity(
            winner.Items.Single().ProductId,
            Quantity.Create(2).Value,
            InfrastructureTestData.Now);
        await repository.SaveAsync(winner, default);
        InfrastructureTestData.MakePaid(stale, transactionId);
        InfrastructureTestData.MakePaid(valid, transactionId);

        (await repository.SavePaymentAsync(stale, transactionId, default))
            .Error.Code.Should().Be("order.version_conflict");
        var validSave = await repository.SavePaymentAsync(
            valid, transactionId, default);
